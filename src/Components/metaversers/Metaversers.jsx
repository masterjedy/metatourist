import { Cell, Group, PanelHeader, Search, Footer, platform, Platform } from '@vkontakte/vkui'
        ...
        <React.Fragment>
            ...
            <Group className='meta-group'>
                ...
                {thematicsFiltered.length > 0 &&
                    thematicsFiltered.map((thematics) => <Cell key={thematics.id}><a className='text-list' href={thematics.url}>{thematics.name}</a></Cell>)}
                {thematicsFiltered.length === 0 && <Footer className='notFaund'>Ничего не найдено</Footer>}
            </Group>
        </React.Fragment>
    )
}

export default Metaversers